...
template <class T>
T& VectorWithOffset<T>::operator[](int i) {
  this->check_state();
...
/*! Out of range errors are detected using assert() */
template <class T>
const T& VectorWithOffset<T>::operator[](int i) const {
  this->check_state();
  assert(i >= this->get_min_index());
  assert(i <= this->get_max_index());
...
template <class T>
inline VectorWithOffset<T> VectorWithOffset<T>::operator*(const VectorWithOffset& v) const {
  this->check_state();
  VectorWithOffset retval(*this);
  return retval *= v;
}
