...
Succeeded
BinNormalisationFromGEHDF5::set_up(const shared_ptr<ProjDataInfo>& proj_data_info_ptr_v) {
...
BinNormalisationFromGEHDF5::read_norm_data(const string& filename) {
  ...
  if (this->use_geometric_factors()) {
    // Construct a proper ProjDataInfo to initialize geometry factors array and use it to know the boudns of the iteratios to
    // load it.
...
float
BinNormalisationFromGEHDF5::get_bin_efficiency(const Bin& bin, const double start_time, const double end_time) const {
