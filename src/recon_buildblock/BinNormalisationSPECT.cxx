...
Succeeded
BinNormalisationSPECT::set_up(const shared_ptr<const ProjDataInfo>& proj_data_info_ptr_v) {
...
void BinNormalisationSPECT::read_norm_data(const std::string& filename) { // to think about this
}

float
BinNormalisationSPECT::get_bin_efficiency(const Bin& bin, const double start_time, const double end_time) const {
    ...
    for (bin.axial_pos_num() = iter->get_min_axial_pos_num(); bin.axial_pos_num() <= iter->get_max_axial_pos_num();
      ...
      for (bin.tangential_pos_num() = iter->get_min_tangential_pos_num();
        ...
        (*iter)[bin.axial_pos_num()][bin.tangential_pos_num()] /=
            (std::max(1.E-20F, get_bin_efficiency(bin, start_time, end_time)) * normalisation);
        normalisation = 1;
      }
  }
}
...
void BinNormalisationSPECT::read_uniformity_table(
