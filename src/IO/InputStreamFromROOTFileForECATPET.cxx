...
InputStreamFromROOTFileForECATPET::InputStreamFromROOTFileForECATPET(std::string _filename, std::string _chain_name,
                                                                     int crystal_repeater_x, int crystal_repeater_y,
                                                                     int crystal_repeater_z, int block_repeater_y,
                                                                     int block_repeater_z, bool _exclude_scattered,
                                                                     bool _exclude_randoms, float _low_energy_window,
    ...
    : base_type(), crystal_repeater_x(crystal_repeater_x), crystal_repeater_y(crystal_repeater_y),
      crystal_repeater_z(crystal_repeater_z), block_repeater_y(block_repeater_y), block_repeater_z(block_repeater_z) {
  set_defaults();
...
InputStreamFromROOTFileForECATPET::get_next_record(CListRecordROOT& record) {
  while (true) {
    ...
    if (eventID1 != eventID2 && exclude_randoms)
      continue;
    if (energy1 < low_energy_window || energy1 > up_energy_window || energy2 < low_energy_window || energy2 > up_energy_window)
      continue;
  ...
  int crystal1 = (blockID1 % block_repeater_y) * crystal_repeater_y + (crystalID1 % crystal_repeater_y);

  int crystal2 = (blockID2 % block_repeater_y) * crystal_repeater_y + (crystalID2 % crystal_repeater_y);
