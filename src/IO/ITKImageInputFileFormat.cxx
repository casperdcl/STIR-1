...
read_file_itk(const std::string& filename) {
  ...
  try {
    if (!dicomIO->CanReadFile(filename.c_str())) {
      ...
      // We've found all "series" (i.e. different data-sets according to above restrictions). Now see which one we should
      // read. We do this by checking which one contains the original filename.
